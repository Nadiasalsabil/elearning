               ...
               header("Location:./index.php");
                ...
                if (isset($_SESSION['username']))
                  {   
                    ...
                    $query  = mysqli_query($konek, "SELECT login.username, nilai.nis,nilai.smt, nilai.nama,nilai.mapel, nilai.absen, nilai.tugas, nilai.uts, nilai.uas, nilai.rata    FROM nilai, login WHERE nilai.smt='2' AND nilai.nis = login.username AND login.username = '".$_SESSION['username']."' ")or die(mysqli_error($konek));
          ...
          include('../config/koneksi.php');
               ...
               header("Location:./index.php");
                ...
                if (isset($_SESSION['username']))
                  {   
                    include '../config/koneksi.php';
           $sql   = "SELECT nilai.mapel  FROM nilai,login where smt='2' AND nilai.nis = login.username AND login.username = '".$_SESSION['username']."' ";
            $query = mysqli_query($konek, $sql )  or die(mysql_error($konek));
            while( $ret = mysqli_fetch_array( $query ) ){
              $merek=$ret['mapel'];                     
                 $sql_jumlah   = "SELECT nilai.rata FROM nilai,login WHERE nilai.mapel='$merek' AND smt='2' AND nilai.nis = login.username AND login.username = '".$_SESSION['username']."' ";        
